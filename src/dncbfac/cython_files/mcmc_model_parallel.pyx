    ...
    cdef void _initialize_state(self, dict state={}):
        ...
        for key, val, update_func in self._get_variables():
            if key in state.keys():
                ...
                update_func(self, update_mode=self._INITIALIZE_MODE)
                ...
                if schedule[k](n):
                    if (verbose > 0) and ((n + 1) % verbose == 0):
                        with timeit_context('sampling %s' % k):
                            update_func(self, update_mode=self._INFER_MODE)
                    else:
                        update_func(self, update_mode=self._INFER_MODE)
            self._total_itns += 1
            if (verbose > 0) and ((n + 1) % verbose == 0):
    ...
    def _test(self, n_samples, method='geweke', var_funcs={}, schedule={}):
        ...
        if method == 'schein':
            for n in range(n_samples):
                ...
                verbose = 10 if n % 500 == 0 else 0
                self._update(10, verbose, schedule)
                # print('updating data')
                self._generate_data()
                # print('calcing funcs')
        ...
        for key, _, _ in self._get_variables():
            if any(schedule[key](n) for n in xrange(n_samples)):
                pass
		# TODO: Put this back in when we have pp_plot
