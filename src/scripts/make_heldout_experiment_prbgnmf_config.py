...
name = "heldout_experiment_prbgnmf"
...
params = it.product(K_list, maskseed_list, modelseed_list, data_path)
# params = it.filterfalse(lambda x: x[0] > x[1], params )
params_df = pd.DataFrame(params, columns=['K','mask_seed', 'seed', 'data_path'])
...
with open('src/scripts/submit_heldout_experiment_prbgnmf.j2', 'r') as f:
