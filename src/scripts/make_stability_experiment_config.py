...
params = it.product(C_list, K_list, modelseed_list, data_path)
# params = it.filterfalse(lambda x: x[0] > x[1], params )
params_df = pd.DataFrame(params, columns=['C','K','seed', 'data_path'])
...
with open('src/scripts/submit_stability_experiment.j2', 'r') as f:
